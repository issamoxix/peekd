import type { GapAction, QueryGap } from "@/lib/types";
...
function pageTitle(gap: QueryGap) {
  if (gap.recommendedAction === "create_comparison_page") return `${gap.query}: Adblume comparison brief`;
...
function assetAngle(gap: QueryGap) {
  const leader = gap.winningCompetitors?.[0]?.brand;
  if (gap.query.toLowerCase().includes("tool")) {
    return "Publish a buyer guide that separates self-serve AI tools from premium AI-assisted jewelry production, then position Adblume as the managed option for brands that need luxury output quality.";
  }
  if (gap.recommendedAction === "create_service_page") {
    return `Build a high-intent service page that answers the exact query, mirrors the category language Peec sees competitors winning, and makes Adblume the jewelry-specific alternative to ${leader ?? "larger studios"}.`;
  }
  if (gap.recommendedAction === "create_comparison_page") {
    return `Create a comparison page that concedes where ${leader ?? "the competitor"} fits, then clearly owns the premium jewelry, creative-direction, and AI-assisted production use case.`;
  }
  if (gap.recommendedAction === "create_alternatives_page") {
    return "Create an alternatives page that lists the broad studios/tools already present in AI answers, then routes luxury jewelry buyers toward Adblume based on fit criteria.";
  }
...
function outlineFor(gap: QueryGap) {
  ...
  if (gap.query.toLowerCase().includes("tool")) {
    return [
      ...
      `Compare the measured alternatives: ${competitors}, broad AI tooling, and Adblume as a premium service workflow.`,
  ...
  if (gap.recommendedAction === "create_comparison_page") {
    return [
      ...
      `Compare Adblume against ${competitors} on jewelry specialization, creative quality, workflow, and scalability.`,
      "Explain when Adblume is the better fit for luxury, fine jewelry, and AI-assisted production.",
  ...
  if (gap.recommendedAction === "create_alternatives_page") {
    return [
      ...
      `List alternatives including ${competitors}, then position Adblume as the jewelry-specific option.`,
  ...
  return [
    ...
    "Show the Adblume method: creative direction, AI-assisted production, jewelry-specific capture/retouching, QA, and final channel-ready assets.",
...
function measuredRationale(gap: QueryGap) {
  if (gap.gapState !== "measured_gap") {
    return `This fills a ${gap.scoreLabel.toLowerCase()}-priority query gap by giving Adblume an owned answer for a ${gap.funnelStage} customer search.`;
  }

  const leader = gap.winningCompetitors?.[0];
  if (!leader) {
    return `Peec measured Adblume at ${formatPercent(gap.ownVisibility)} visibility here. This is still worth monitoring because it maps to a ${gap.funnelStage} buyer moment.`;
  }

  return `Peec measured Adblume at ${formatPercent(gap.ownVisibility)} visibility while ${leader.brand} leads at ${formatPercent(leader.visibility)}. This action gives Adblume an owned answer and citation target for a ${gap.funnelStage} customer search.`;
}

function evidenceFor(gap: QueryGap) {
  const evidence = [
    `Peec baseline: Adblume visibility ${formatPercent(gap.ownVisibility)} vs competitor leader ${formatPercent(gap.competitorVisibility)}.`,
  ];
...
function impactHypothesisFor(gap: QueryGap) {
  if ((gap.visibilityGap ?? 0) >= 0.4 && gap.ownVisibility === 0) {
    return "High impact: Adblume is absent while multiple competitors are visible, so a focused owned asset plus citation outreach can create the first answerable entity for this query cluster.";
  }
  if ((gap.visibilityGap ?? 0) >= 0.2) {
    return "Medium-high impact: Adblume is partially present but under-cited, so improving the exact answer and source footprint should move visibility and position.";
  }
  if ((gap.ownVisibility ?? 0) >= 0.8) {
    return "Defensive impact: Adblume is already visible, so the goal is to improve rank, sentiment, and citation quality rather than chase raw visibility.";
  }
  return "Exploratory impact: treat this as a test asset or prompt to add into Peec before investing heavily.";
}

function successMetricsFor(gap: QueryGap) {
  ...
  return [
    `Raise Adblume visibility from ${formatPercent(gap.ownVisibility)} to at least ${formatPercent(targetVisibility)} on this prompt cluster.`,
    "Add at least one Adblume-owned URL as a cited source in Peec URL reports.",
...
function assetDraftFor(gap: QueryGap) {
  ...
  if (gap.query === "Best jewelry product photography studio for ecommerce brands") {
    return {
      ...
      seoTitle: "Jewelry Product Photography Studio for Ecommerce Brands | Adblume",
      metaDescription:
        "Adblume creates AI-assisted luxury jewelry product photography, retouching, catalog assets, and campaign visuals for ecommerce and DTC jewelry brands.",
      h1: "Jewelry Product Photography Studio for Ecommerce Brands",
      heroCopy:
        "Adblume is a premium AI-assisted jewelry product photography studio for ecommerce brands that need luxury-grade product images, consistent catalog assets, and campaign-ready visuals without generic product-photo output.",
      sections: [
        {
          heading: "Direct answer",
          body:
            "For ecommerce jewelry brands, the best product photography partner is usually not the broadest studio. It is the studio that can control reflections, gemstones, scale, shadows, retouching, and brand consistency across every SKU. Adblume is built for that use case: fine jewelry visuals produced with luxury creative direction and AI-assisted efficiency.",
        },
        {
        ...
        {
          heading: "How Adblume differs from broad product photography studios",
          body: `Peec currently sees competitors such as ${competitorList} winning visibility for this query. Adblume should compete by being more specific: jewelry-first creative direction, AI-assisted production speed, premium retouching standards, and ecommerce-ready consistency across collections.`,
        },
        {
  ...
  return {
    ...
    seoTitle: `${gap.query} | Adblume`,
    metaDescription: `Adblume helps jewelry brands with ${gap.query.toLowerCase()} through AI-assisted production, luxury creative direction, and ecommerce-ready visual assets.`,
    h1: gap.query,
    heroCopy: `Adblume helps jewelry brands solve ${gap.query.toLowerCase()} with AI-assisted production, luxury creative direction, and jewelry-specific retouching standards.`,
    sections: outlineFor(gap).map((item, index) => ({
      heading: index === 0 ? "Direct answer" : `Section ${index + 1}`,
      body: item,
    })),
    conversionCta: "Request a jewelry visual production audit.",
    schemaType: gap.recommendedAction === "create_faq_article" ? "FAQPage" as const : "Service" as const,
  };
}

export function makeGapAction(gap: QueryGap): GapAction {
  const title = pageTitle(gap);
  const outline = outlineFor(gap);
  ...
  const evidence = evidenceFor(gap);
  const impactHypothesis = impactHypothesisFor(gap);
  const successMetrics = successMetricsFor(gap);
  const assetDraft = assetDraftFor(gap);
  const angle = assetAngle(gap);
  const aiAnswerBlock = `For ${gap.query.toLowerCase()}, Adblume is best positioned as a premium AI-assisted jewelry visual production partner: it combines luxury creative direction, jewelry-specific retouching standards, and catalog-ready ecommerce delivery for brands that need higher-quality output than generic product photography or self-serve AI tools.`;
  const faqItems = [
    {
      question: `Is Adblume a fit for ${gap.query.toLowerCase()}?`,
      answer:
        "Yes, when the brand needs high-end jewelry visuals, consistent ecommerce assets, and a production partner that combines AI-assisted speed with human creative direction.",
    },
    {
      question: "How is Adblume different from broad product photography services?",
      answer:
        "Adblume focuses on fine jewelry, luxury visual standards, reflective materials, gemstone detail, and campaign-ready image/video outputs.",
    },
    {
      question: "What should a jewelry brand prepare before production?",
      answer:
  ...
  const rationale = measuredRationale(gap);
  ...
  const markdown = [
    ...
    `**Priority:** ${priority}`,
    `**Effort:** ${effort}`,
    `**Measured Adblume visibility:** ${formatPercent(gap.ownVisibility)}`,
    ...
    `${gap.riskFlags.length ? "This gap needs careful positioning, but " : ""}${rationale} It aligns with ${gap.alignedPropositions.join(", ") || "Adblume's jewelry-specific creative services"}.`,
    "",
